...
import { supabase } from "../lib/supabase"; // ajusta se seu caminho for diferente
...
function Badge({ kind }: { kind: BadgeType }) {
  ...
  if (kind === "elite") {
    return (
      <span
        className={`${styles.badge} ${styles["badge--elite"]}`}
      >
        Elite
      </span>
    );
  ...
  return (
    <span className={`${styles.badge} ${styles["badge--pro"]}`}>
      Pro
    </span>
  );
}
