...
# =========================================================
# This master script validates EVERYTHING in one command
# Run with: .\validate-all.ps1
# =========================================================
...
if (-not (Test-Path "masseurmatch-nextjs")) {
...
try {
    $supabaseScript = Join-Path $PSScriptRoot "validate-supabase.ps1"
    & $supabaseScript
...
try {
    $frontendScript = Join-Path $PSScriptRoot "validate-frontend.ps1"
    & $frontendScript
    if ($LASTEXITCODE -eq 0) {
