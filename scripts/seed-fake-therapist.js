...
// Carregar variáveis de ambiente do .env.local manualmente (sem depender de dotenv)
const fs = require('fs');
...
// Configuração do Supabase (usando service_role_key para criar usuários)
...
const therapistData = {
  ...
  about: 'Olá! Sou Alex Santos, massagista certificado com mais de 8 anos de experiência ajudando clientes a alcançarem relaxamento profundo e alívio de tensões musculares. Minha abordagem combina técnicas tradicionais com métodos modernos para proporcionar uma experiência única e terapêutica.',
  philosophy: 'Acredito que a massagem é uma arte de cura que vai além do físico, alcançando o bem-estar mental e emocional. Cada sessão é personalizada para atender às necessidades específicas de cada cliente, criando um ambiente seguro e acolhedor.',
  ...
  services: ['Massagem Terapêutica', 'Massagem Relaxante', 'Massagem Desportiva', 'Deep Tissue', 'Swedish Massage'],
  massage_techniques: ['Deep Tissue', 'Swedish', 'Sports Massage', 'Trigger Point Therapy', 'Myofascial Release', 'Hot Stone', 'Aromatherapy'],
  studio_amenities: ['Ambiente climatizado', 'Música relaxante', 'Aromaterapia', 'Chuveiro disponível', 'Estacionamento gratuito', 'Toalhas limpas fornecidas'],
  mobile_extras: ['Mesa portátil profissional', 'Óleos premium', 'Música ambiente', 'Toalhas aquecidas'],
  additional_services: ['Reflexologia', 'Esfoliação corporal', 'Terapia com pedras quentes'],
  products_used: 'Óleos essenciais orgânicos, Óleos de massagem premium sem fragrância',
  rate_60: '$80',
  rate_90: '$110',
  rate_outcall: '$150 (+ taxa de deslocamento)',
  payment_methods: ['Dinheiro', 'Cartão de crédito', 'Cartão de débito', 'Venmo', 'Zelle', 'PayPal'],
  ...
  degrees: 'Certificação em Massoterapia pelo Pacific College of Health and Science (2016)',
  affiliations: ['AMTA - American Massage Therapy Association', 'NCBTMB - National Certification Board'],
...
const fakeReviews = [
  {
    ...
    rating: 5,
    comment: 'Experiência incrível! Alex tem mãos mágicas e realmente entende de técnicas de massagem profunda. Saí me sentindo renovada!',
    date: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString()
  },
  {
    reviewer_name: 'John Williams',
    rating: 5,
    comment: 'Best massage I\'ve had in LA! Very professional, clean studio, and Alex really knows how to work out those knots. Highly recommend!',
    date: new Date(Date.now() - 12 * 24 * 60 * 60 * 1000).toISOString()
  },
  {
    reviewer_name: 'Carlos Rodriguez',
    rating: 4,
    comment: 'Muito bom! Atendimento excelente e ambiente super relaxante. A única coisa é que gostaria que as sessões fossem um pouco mais longas.',
    date: new Date(Date.now() - 18 * 24 * 60 * 60 * 1000).toISOString()
  },
  {
    reviewer_name: 'Sarah Johnson',
    rating: 5,
    comment: 'Alex is amazing! I have chronic back pain and after just 3 sessions I\'m feeling so much better. The hot stone add-on is worth it!',
    date: new Date(Date.now() - 25 * 24 * 60 * 60 * 1000).toISOString()
  },
  {
    reviewer_name: 'Pedro Mendes',
    rating: 5,
    comment: 'Profissional top! Explica tudo antes de começar, respeita os limites e realmente se preocupa com o bem-estar do cliente.',
...
async function main() {
  ...
  try {
    ...
    const userExists = existingUser?.users?.some(u => u.email === FAKE_USER_EMAIL);

    if (userExists) {
      console.log('⚠️  Usuário já existe. Deletando para recriar...');

      // Buscar o ID do usuário existente
      const existingUserId = existingUser.users.find(u => u.email === FAKE_USER_EMAIL)?.id;
    ...
    if (therapistError) {
      console.error('❌ Erro ao criar perfil de massagista:', therapistError.message);
    ...
    if (reviewsError) {
      console.warn('⚠️  Aviso ao criar reviews (tabela pode não existir):', reviewsError.message);
    } else {
